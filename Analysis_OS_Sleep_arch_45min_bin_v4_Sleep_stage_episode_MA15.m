...
addpath(genpath('\\genzel-srv.science.ru.nl\genzel\Rat\OS\OS_rat_ephys\sleep_architecture_scripts_NSD\Scripts for NSD'))
...
subtrial=[0 0 0 0 0 1 2 3 4 0 0];
for i=1:length(rats)
    cd(rats{i})
    SD=getfolder;
         for ii=1:length(SD)
           ...
           % treatment_I = [];
            ...
            subtrial_R = [];
                    ...
                    for iii=1:length(files)
                        ...
                        file=files{iii};
                        load(file);
                        [new_states]=corrected_states(states); %Correct 4s bouts duration for all sleep stages
                        [episode_states]=merge_states(new_states); %Merge bouts from the same stage separate by <=300s wake 

                        Length=length(episode_states); %Fill in the vector with NaN (empty cells) in case the vector is shorter than 2700s  or 10800s (pt5 and pt6). Split Pt5 and pt6 in 4 vectors of 2700s each, and combine all the resting periods in a matrix (SD_matrix) that we use for all the analysis
                          if iii~=6 && iii~=8   %If PS, PT1-4, PST
                              if (Length < 2700);
                                 a_45min = [episode_states nan(1,2700-length(episode_states))];
                              else 
                                 a_45min = episode_states(1:2700);
                              end
                              SD_matrix=[SD_matrix; a_45min];                          
                          else          %Split Post_Trial5 and Post_Trial6 in 4 bins of 45 min
                              if (Length < 10800)
                                 a_45min = [episode_states nan(1,10800-length(episode_states))];
                              else 
                                 a_45min = episode_states(1:10800);
                              end
...
%                                               
                ...
                % and the rest of values with zeros. The final variable bouts_REM contains the duration of  each bout of REM (Area) and the index in the original state vector (PixelIxList)  
                 ...
                 save('Sleep_architecture_45min_nc.mat', 'animal', 'study_day', 'condition',  'condseq', 'file_name', 'trial_name','trial', 'Awake_s', 'NREM_s', 'Intermediate_s', 'REM_s', 'Ttime_s', 'TST_s',...
                     ...
                     'recording_A', 'animal_A',  'condition_A', 'condseq_A', 'trial_A', 'subtrial_A', 'Awake_bouts_c', 'Awake_bouts_d', 'Awake_bouts_s', 'Awake_bouts_e',...
                     'recording_MA', 'animal_MA',  'condition_MA', 'condseq_MA', 'trial_MA', 'subtrial_MA', 'MA_bouts_c', 'MA_bouts_d', 'MA_bouts_s', 'MA_bouts_e',...
                     'recording_N', 'animal_N',  'condition_N', 'condseq_N', 'trial_N', 'subtrial_N', 'NREM_bouts_c', 'NREM_bouts_d', 'NREM_bouts_s', 'NREM_bouts_e',...
                     'recording_I', 'animal_I', 'condition_I', 'condseq_I', 'trial_I', 'subtrial_I', 'Intermediate_bouts_c', 'Intermediate_bouts_d', 'Intermediate_bouts_s', 'Intermediate_bouts_e',...
                     'recording_R', 'animal_R',  'condition_R', 'condseq_R', 'trial_R', 'subtrial_R', 'REM_bouts_c', 'REM_bouts_d', 'REM_bouts_s', 'REM_bouts_e',...
