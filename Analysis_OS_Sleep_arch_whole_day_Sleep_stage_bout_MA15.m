...
%Before running this script, run 'concatenate_states' to merge the whole
%study day which creates Training_merged.mat and Test_merged.mat. 
%For the training and the Test file this script requieres to be run
%individually.
    ...
    SD=getfolder;
         ...
         for ii=1:length(SD)
             ...
             condseq=[condseq; seq];
                         ...
                         Awake_1_s = [sum(concatenated_states == 1)];
                         Awake_s = [Awake_s; Awake_1_s];
                         NREM_1_s = [sum(concatenated_states == 3)];
                         NREM_s = [NREM_s; NREM_1_s];
                         Intermediate_1_s =[sum(concatenated_states == 4)];
                         Intermediate_s =[Intermediate_s; Intermediate_1_s];
                         REM_1_s = [sum(concatenated_states == 5)];
                            REM_s = [REM_s; REM_1_s];
                            Ttime_1_s =length(concatenated_states)-sum(isnan(concatenated_states));
                    ...
                    %Sleep stages transitionsn start with 0
                         for iii=1:length(episode_states)-1   
                             ...
                             count_rem2inter=0;
                                       %wake2nrem
                                       if concatenated_states(iii) == 1 && concatenated_states(iii+1)==3
                                           count_wake2nrem=count_wake2nrem+1;
                                        end
                                                                   
                                        %wake2inter (it is not common, but we could use it as control)                                    %scripts)
                                        if concatenated_states(iii) == 1 && concatenated_states(iii+1)==4
                                            count_wake2inter=count_wake2inter+1;
                                        end
                                     %wake2rem (it is not common, but we could use it as control)
                                        if concatenated_states(iii)==1 && concatenated_states(iii+1)==5
                                            count_wake2rem=count_wake2rem+1;
                                        end
                                     %nrem2inter
                                        if concatenated_states(iii)==3 && concatenated_states(iii+1)==4
                                             count_nrem2inter=count_nrem2inter+1;
                                        end
                                     %nrem2rem
                                        if concatenated_states(iii)==3 && concatenated_states(iii+1)==5
                                             count_nrem2rem=count_nrem2rem+1;
                                        end
                                     %nrem2awake
                                        if concatenated_states(iii)==3 && concatenated_states(iii+1)==1
                                             count_nrem2wake=count_nrem2wake+1;
                                       end  
                                     %inter2rem
                                        if concatenated_states(iii)==4 && concatenated_states(iii+1)==5
                                             count_inter2rem=count_inter2rem+1;
                                        end
                                     %inter2awake
                                        if concatenated_states(iii)==4 && concatenated_states(iii+1)==1
                                              count_inter2wake=count_inter2wake+1;
                                        end
                                     %inter2nrem
                                        if concatenated_states(iii)==4 && concatenated_states(iii+1)==3
                                              count_inter2nrem=count_inter2nrem+1;
                                        end
                                      %rem2awake
                                        if concatenated_states(iii)==5 && concatenated_states(iii+1)==1
                                              count_rem2wake=count_rem2wake+1;
                                        end
                                      %rem2inter
                                        if concatenated_states(iii)==5 && concatenated_states(iii+1)==4
                                              count_rem2inter=count_rem2inter+1;
                                        end
                                      %rem2nrem
                                        if concatenated_states(iii)==5 && concatenated_states(iii+1)==3
                              ...
                              Trial_bin = concatenated_states == 7;
...
%                                         subtrial_A = [subtrial_A; repelem(subtrial(iiii),Trial_bouts_count1,1)];
                              ...
                              Wake_bin = concatenated_states == 1;
...
%                     q                    
                ...
                % and the rest of values with zeros. The final variable bouts_REM contains the duration of  each bout of REM (Area) and the index in the original state vector (PixelIxList)  
                            ...
                            NREM_bin = concatenated_states==3;
...
%                                         subtrial_N = [subtrial_N; repelem(subtrial(iiii),NREM_bouts_count1,1)];
                            ...
                            Intermediate_bin = concatenated_states==4;
